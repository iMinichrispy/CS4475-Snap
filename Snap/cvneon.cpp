...
namespace cv
{
  ...
  {
    const size_t continuosStep = source.cols * source.elemSize();
    const size_t alignedStep   = (continuosStep / 16 + (continuosStep % 16 > 0 ? 1 : 0)) * 16;
  ...
  void align16(const cv::Mat& m, cv::Mat& aligned)
  {
    ...
    const size_t continuosStep = m.cols * m.elemSize();
    const size_t alignedStep   = (continuosStep / 16 + (continuosStep % 16 > 0 ? 1 : 0)) * 16;
